...
request({
        ...
        if(err || res.statusCode !== 200)
            return console.log(chalk.red('[%d] %s', res.statusCode, err));

        registry = body;
        console.log(chalk.white('Initial registry saved'));
}).pipe(fs.createWriteStream('oldreg.json'));
...
function compileDatabase(checksum, modified) {
    ...
    req.on('end', function(err) {
        console.log(chalk.green('2. Success!');
